...
from constants import TMP_DRIVE_AUTH, SIXGPT_API
...
def get_active_account() -> T.Optional[Credentials]:
    if os.path.exists(TMP_DRIVE_AUTH):
        ...
        if creds.expired:
            ...
            click.echo("Refreshing drive auth token...")
            click.echo(f"Current token: {creds}")
...
def _call_volara_api_server() -> T.Optional[Credentials]:
    url_response = requests.get(f"{SIXGPT_API}/drive/get-url")
    ...
    code_response = requests.get(f"{SIXGPT_API}/drive/callback?code={code}")
...
def _call_volara_api_server_refresh(creds: Credentials) -> T.Optional[Credentials]:
    url_response = requests.get(
        f"{SIXGPT_API}/drive/refresh-token",
        params={
